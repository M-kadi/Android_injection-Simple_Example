...
public final class UserDao_Impl implements UserDao {
  ...
  public UserDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfUser = new EntityInsertionAdapter<User>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR ABORT INTO `users` (`id`,`usr_name`) VALUES (nullif(?, 0),?)";
      }

      @Override
      public void bind(SupportSQLiteStatement stmt, User value) {
        stmt.bindLong(1, value.getId());
        if (value.getUsr_name() == null) {
          stmt.bindNull(2);
        } else {
          stmt.bindString(2, value.getUsr_name());
        }
      }
    };
  ...
  public Flow<List<User>> getAllUsers() {
    final String _sql = "SELECT * from users ORDER BY id Desc";
      ...
      public List<User> call() throws Exception {
        ...
        try {
          ...
          while(_cursor.moveToNext()) {
            final User _item;
            final String _tmpUsr_name;
            _tmpUsr_name = _cursor.getString(_cursorIndexOfUsrName);
            _item = new User(_tmpUsr_name);
            final int _tmpId;
